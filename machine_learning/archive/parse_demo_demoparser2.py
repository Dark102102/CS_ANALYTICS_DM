...
def parse_demo_with_demoparser2(dem_path: Path):
    ...
    try:
        ...
        try:
            ...
            print(f"Failed to initialize parser for {dem_path.name}: {parser_error}")
            # Create error CSV
            df = pd.DataFrame([{
                'round_num': None,
                'winning_team': None,
                ...
                'demo_type': 'cs2',
                'map_name': 'unknown',
                'parsing_status': f'parser_init_error: {str(parser_error)[:100]}'
            }])
        ...
        try:
            ...
            print(f"Failed to parse header: {header_error}")
